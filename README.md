...
光放行协议只解决了「连得上」，机器人仍然只能看到原版方块。服务端启动时会额外导出一份
**注册表快照**：

```text
<服务端根目录>/botcompat-registry.json
```

内容包括（1.21.1 + 41 个模组的实测数字）：

| 字段 | 内容 | 实测 |
| --- | --- | --- |
| `vanilla_max_block_state_id` | 服务端所见原版状态上限 | 26683 |
| `blocks[]` | 非原版方块状态：ID、方块名、状态串、是否空气、硬度、是否需要正确工具、是否流体、**碰撞箱列表** | 50,090 条 / 1,260 种方块 |
| `items[]` | 非原版物品：协议 ID + 名字 | 1,406 条 |
| `entity_types[]` | 非原版实体类型 | 20 条 |
| `block_items[]` | 方块 → 能放置它的物品（`BlockItem`），用于机器人**放置模组方块** | 2,084 条（含原版） |

把这份文件交给机器人侧（配套的 Rust 项目会加载它），机器人就能把数值 ID 还原成
**名字 + 真实碰撞箱 + 硬度**。实测效果：机器人世界里 139,623 个模组方块实例全部识别出名字，
并能按名字挖掉（例如 `mine biomesoplenty:high_grass_plant`），也能按名字**放置**（`build create:cogwheel x y z x y z`：
机器人据此找到快捷栏里的对应物品、切换选中并右键）。

为什么必须导出而不能靠协议拿：原版协议里 `minecraft:block` 注册表是**内置**的，
服务端假定「客户端和自己注册表一致」，直接下发数值 ID，从不发送方块名。
